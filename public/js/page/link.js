$(document).ready(function() {
        ...
        $('body').on('click','.edit-link',function(event) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    $('#add-form').find('#link').val(result.data.link);                    
                }
            });    
        });
        ...
        if($('#linkTable').length>0)
        var departmentTable = $('#linkTable').DataTable({
            searching: true,
            pageLength: 25,
            processing: true,
            serverSide: true,
            ...
            columns: [
                { data: 'name' },
                {
                    ...
                    render: function(_,_, full) {
                        ...
                        if(link) {
                            actions = "";
                            actions += ' <a href="'+link+'" target="__blank">'+link+'</a>';                            
                            return actions;
                        }
                        return '';
                    }, 
                },
                {
                    ...
                    render: function(_,_, full) {
                        ...
                        if(contactId) {
                            actions = "";
                            actions += ' <a href="javascript:void(0)" data-id="'+ contactId +'" class="btn-sm btn-warning edit-link"><i class="mdi mdi-pencil"></i></a>';
                            actions += ' <a href="javascript:void(0)" data-id="'+ contactId +'" class="btn-sm btn-danger delete-link"><i class="mdi mdi-trash-can"></i></a>';
